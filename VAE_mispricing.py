...
data.dropna(subset=X_vars + ['mispricing', 'return'], inplace=True)
...
data['mispricing'] = scaler_T.fit_transform(data[['mispricing']])
...
data['mispricing'] = data['mispricing'].clip(-5, 5)
data['return'] = data['return'].clip(-5, 5)

class FinanceDataset(Dataset):
    def __init__(self, df):
        self.X = df[X_vars].values.astype(np.float32)
        self.T = df['mispricing'].values.astype(np.float32).reshape(-1, 1)
